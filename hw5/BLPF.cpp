...
void BLPF(unsigned char* input_img, unsigned char* output_img, unsigned char* spectrum_img, int width, int height, int r, int nn) {
	/*********************img - processing - DFT*********************/
	...
	for (int x = 0; x < height; ++x) {
		for (int y = 0; y < width; ++y) {
			...
			filter[x * width + y] = 1.0 / (1.0 + pow( (D_uv + 1e-6) / r, 2 * nn));
		}
	}
