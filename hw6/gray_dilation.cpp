...
void gray_dilation(unsigned char* input_img, unsigned char* output_img, int width, int height, int num) {
	int half_num = num / 2;
	...
	for (int x = half_num; x < height + half_num; ++x) {
		for (int y = half_num; y < width + half_num; ++y) {
			...
			output_img[(x - half_num) * width + (y - half_num)] = max >= 255 ? 255 : max;
		}
	}
}